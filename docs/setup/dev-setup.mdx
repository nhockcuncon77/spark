...
description: Get your local environment ready to build, run, test, and debug Spark across backend, database, and mobile app.
icon: "toolbox"
---
...
<Steps>
  <li>
    ...
    ```bash
    git clone https://github.com/nhockcuncon77/spark.git
    cd spark
    ```
  </li>
  <li>
...
Create a `.env` in the project root (`spark/.env`) for runtime secrets used by the backend. Examples:
  ...
  <Tab title="Using local Postgres">
    <Steps>
      ...
      <li>
        ...
        export DB_USER=spark
        export DB_PASSWORD=spark_password
        export DB_NAME=spark
        export DB_PORT=5432
        export DB_SSL=false
        ```
      </li>
...
<CardGroup cols={2}>
  <Card title="Self-host Spark" icon="rocket" href="/docs/setup/self-host">
