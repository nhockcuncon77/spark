---
title: Self-host Spark
description: Deploy Spark using Docker Compose or Kubernetes, and set up the mobile app via Expo, APKs, or app stores.
icon: "rocket"
---
...
You can self-host Spark in a few ways:
...
<Steps>
  <li>
    Create a `.env` file in the repository root (`spark/.env`) with any required secrets and configs used by the backend. The Compose file already sets:
    <ul>
      <li><code>DATABASE_URL</code> to <code>postgres://spark:spark_password@postgres:5432/spark?sslmode=disable</code></li>
...
Use the provided deployment manifest to run Spark on a Kubernetes cluster. It includes:
...
- TLS is configured for your domain (replace with your domain and secret)
- The backend image — update this to your registry if needed.
